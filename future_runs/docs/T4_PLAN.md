...
> **2026-09-08 修订（用户决定）：正式布局定为 20 节点 / 2560 tasks /
> 每节点 200g，最多 3 个情景并发（60 节点）。T4 取消 10 节点对照**，
> 只保留 20 节点两年生产配置测试。
>
> **T4 的目的因此从"选布局"变成"验证已选的布局"**：
> ① 200g 是否够（看峰值 `anon` 余量与 `oom_kill`）；
> ② 稳定吞吐（模式年/小时）；
> ③ I/O 代价（月度写出、文件关闭、restart 写出）；
> ④ **据此确定正式运行的分段长度与 walltime**。
>
> T4 通过后在同一 20 节点布局上做完整 T3；七个 77 年正式情景仍待最终确认，
> 按 **3 + 3 + 1** 分批提交。
...
（原计划里"两个布局共用一个二进制"的安排随 10 节点对照一并取消。
`NTASKS` 不进 E3SM 构建这一点仍然成立，正式的七个 case 因此可以共用
同一个生产 exe。）
...
| `NTHRDS_*` | 1 |
| `--mem` | **200g** —— 本次要验证的对象 |
| walltime（T4 本身） | 03:00:00 |
| CASEROOT | `…/e3sm_cases/20260908_seus_4km_fut_t4_n20` |
| RUNDIR / EXEROOT | `/scratch/…/cime_output_dirs/20260908_seus_4km_fut_t4_n20/{run,bld}` |

正式运行的并发上限：**3 个情景 × 20 节点 = 60 节点**。对照 §2 末的节点供给
...
1. **200g 够不够**：峰值 `anon` 对 200g 的余量，`oom_kill` 必须为 0。
   用 `anon` 而不是 `memory.current`，理由见 §5。
   若 `anon` 逼近 200g，需要在正式提交前调整档位并重跑本测试。
2. **稳定吞吐**（模式年/小时），据此推算 77 年单 run 的墙钟。
3. **I/O 代价**：月度写出、文件关闭、restart 写出各自的时间占比。
4. **由 ①②③ 定正式分段**：段长（`STOP_N`）与 `RESUBMIT`，以及每段 walltime。
   段长要能整除 77（7 或 11），并留出足够的墙钟余量。
   同时记录：分段边界越多，重复初始化的开销越大（初始化要预载 227,760 条
   气象记录），这个代价由 §6 的初始化读入时间直接给出。

**T4 通过后**：在同一 20 节点布局做完整 T3（连续 vs 分段，中断点取零点），
然后七个正式 run 按 3 + 3 + 1 分批，等最终确认。
