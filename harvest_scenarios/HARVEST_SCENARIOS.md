...
| (参考)全域净差 `Σf1850 − Σf2023` | 994,213 |
| **grass-only 可达目标** | **820,075** = 补上**逐格点缺口的 69.9%** |
| **RF 2050 年后的平台值** | **5,469,839** = 1850 年**森林范围的 96.9%** |

> **分母极易搞错,这个数已经错过两次。** 正确的是
> 820,075/**1,173,576** = **69.9%,分母是逐格点正缺口**;
> 5,469,839/5,643,976 = **96.9%,分母是 1850 年的森林总量**。
> 写成"恢复到 1850 extent 的 82.5%"是**错的**(那个数是 96.9%)。
>
> **另一个更隐蔽的错**:用**全域净差 994,213** 当分母会得到 82.5%,但那是**口径不一致**——
> `full_increment = Σ min(max(f1850−f2023, 0), grass_2023)` 的分子是**逐格点单向**构造的,
> 而净差把"2023 年森林反而比 1850 多"的那 **179,364** 抵消掉了。那些格点缺口为 0、
> RF 根本不会碰,算进分母只会虚高覆盖率。分子单向,分母就必须单向。
