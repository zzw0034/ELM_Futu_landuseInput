...
directory. The old script remains recoverable at git commit dd82b07.
  ...
  RF: a SCENARIO-INDEPENDENT policy prescription, so ONE file serves all
      ...
      GRASS ONLY: cropland is never converted, which closes 69.9% of the
      1850 DEFICIT (820075 of 1173576), leaving RF at 96.9% of the 1850
...
if ALL_SSPS[0] in SSPS and not ONLY_RH:
    ...
    clone_structure(rf_template, out_rf, rf_overrides,
                     ...
                     f"69.9% of the 1850 DEFICIT (820075 of 1173576 in %-of-cell units, where "
                     f"the deficit is the PER-CELL one-sided sum that the target is built from, "
                     f"NOT the domain net difference of 994213), which puts the 2050+ plateau at "
                     f"5469839 = 96.9% of the 1850 forest EXTENT (5643976). Do not conflate "
                     f"those denominators, and do NOT read any of these as km2 -- they are raw "
                     f"sums of PCT_NAT_PFT percent across grid cells, not area-weighted; the "
                     f"true 1850 forest extent is 976420 km2, about 5.8x smaller, via AREA x "
                     f"LANDFRAC_PFT x PCT_NATVEG/100. "
